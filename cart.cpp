...
Cart::Cart()
    : courses(nullptr), schedules(nullptr), course_count(0), total_cost(0.0) {}

Cart::Cart(const Cart &other)
    : courses(nullptr), schedules(nullptr), course_count(other.course_count),
      total_cost(other.total_cost) {
  if (course_count > 0) {
    courses = new Course[course_count];
    schedules = new std::string[course_count];
...
bool Cart::isEmpty() const { return course_count == 0; }

std::size_t Cart::getCourseCount() const { return course_count; }

double Cart::getTotalCost() const { return total_cost; }
